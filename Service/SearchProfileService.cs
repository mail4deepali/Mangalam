            ...
            try
            {
                using (IDbConnection dbConnection = new NpgsqlConnection(_ConnectionStringService.Value))
                {
                    ...
                    using (var transaction = dbConnection.BeginTransaction())
                    {
                        ...
                        List<CandidateDetails> candidates = dbConnection.Query<CandidateDetails>(" select c.id, c.first_name, c.last_name,  c.date_of_birth, ge.gender, r.religion_name as religion, ca.caste_name as caste, null as image from candidate c join gender ge on c.gender_id = ge.id join religion r on c.religion_id = r.id  join caste ca on c.caste_id = ca.id  where date_of_birth between  @p0  and  @p1  and c.id != @p2", new { p0 = ageRange.fromBirthdate, p1 = ageRange.toBirthdate, p2 = ageRange.candidate_id }).ToList();
